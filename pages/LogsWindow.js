...
// Individual Log button
	...
	win.add(buttonIndividualLog);
		...
		var buttonTeamLog = Ti.UI.createButton({
		title: 'Team Log',
		color: '#7139BB',
		backgroundColor: '#D1E5A7',
		font: {
	...
	win.add(buttonTeamLog);
	
	buttonIndividualLog.addEventListener('click', function(){
		var individualLogPage = require('/pages/individualLogPage');
		new individualLogPage().open();
	});
	
	buttonTeamLog.addEventListener('click', function(){
		var teamLogPage = require('/pages/teamLogPage');
		new teamLogPage().open();
	});
